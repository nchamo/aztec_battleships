...
`;
  ...
  console.log(`   ✅ Generated TypeScript wrapper: ${wrapperPath}`);
}
...
function compileLocalContracts(projectRoot: string): boolean {
  ...
  console.log(`\n🔨 Compiling ${contractDirs.length} local contract(s)...`);
  let allSuccess = true;
...
async function main() {
  ...
  console.log(`
╔════════════════════════════════════════════════════════════════╗
║           BUILD BATTLESHIPS CONTRACT                           ║
╚════════════════════════════════════════════════════════════════╝
`);

  try {
    console.log('='.repeat(60));
    console.log('🔨 Compiling Battleships contract');
    console.log('='.repeat(60));

    if (!compileLocalContracts(projectRoot)) {
      console.warn('\n⚠️ Contract compilation had issues');
      process.exit(1);
    } else {
